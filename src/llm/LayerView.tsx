...
export function LayerView() {
  ...
  function handleCopyCamera(ev: React.MouseEvent) {
    ...
    let vecToString = (vec: Vec3) =>
      `new Vec3(${vec.x.toFixed(3)}, ${vec.y.toFixed(3)}, ${vec.z.toFixed(3)})`;

    let cameraStr = `${vecToString(camera.center)}, ${vecToString(
      camera.angle
    )}`;
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      let [data, model, native] = await Promise.all([
        dataP,
        modelP,
        nativeBindingsP,
      ]);
  ...
  let mainView = (
    <div className={s.canvasWrap}>
      ...
      {canvasRender && !canvasRender.progState.render && (
        <div className="absolute flex flex-col items-center w-full h-full justify-center">
          ...
          <div className="text-lg mt-2">
            Please try the latest version of Chrome or Firefox.
          </div>
        </div>
      )}
      ...
      {canvasRender && (
        <ProgramStateContext.Provider value={canvasRender.progState}>
          <CanvasEventSurface>{/* <MovementControls /> */}</CanvasEventSurface>
          {/* <WelcomePopup /> */}
...
class CanvasRender {
  ...
  setData(data: ICanvasData) {
    ...
    if (
      data.dataAndModel &&
      !this.progState.gptGpuModel &&
      this.progState.render
    ) {
      this.progState.gptGpuModel = initModel(
        this.renderState,
        data.dataAndModel,
        1
      );
      ...
      this.progState.wasmGptModel = constructModel(
        data.dataAndModel.model,
        data.dataAndModel.model.config,
        data.dataAndModel.native
      );
